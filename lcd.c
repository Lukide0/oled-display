...
void lcd_home(void) { lcd_gotoxy(0, 0); }
...
void lcd_putc(char c) {
    ...
    case '\t':
        // tab
        if ((unsigned int)(cursorPosition.x + charMode * 4) < (DISPLAY_WIDTH / sizeof(FONT[0]) - charMode * 4)) {
    ...
    default:
        ...
        if (charMode == DOUBLESIZE) {
            ...
            if ((cursorPosition.x + sizeof(FONT[0])) > DISPLAY_WIDTH)
                break;
...
void lcd_charMode(uint8_t mode) { charMode = mode; }
